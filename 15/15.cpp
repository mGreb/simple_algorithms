...
bool isChecking(const tree& t, const std::function<bool(int, int)>& checker) {
    ...
    while (curr != -1 || !s.empty()) {
        ...
        if (!checker(curr, t[(size_t)curr][V])) {
            return false;
        }
...
int main() {
    ...
    const auto checker = [isPrevValSet = false, prevVal = 0, prevInd = 0](int ind, int val) mutable {
        if (isPrevValSet && (val < prevVal || (val == prevVal && ind < prevInd))) {
            return false;
        } else {
            prevVal = val;
            prevInd = ind;
